...
def fttransformation(): 
    try:  
        ...
        columns = defaultdict(list) # each value in each column is appended to a list每列中的每个值都会附加到列表中    
        with open("features_raw.csv") as f:
            reader = csv.DictReader(f) # read rows into a dictionary format将行读入字典格式
            try:
                for row in reader: 
