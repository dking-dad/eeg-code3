...
#fetching current working directory 获取当前工作目录
path=  os.getcwd() 
path+="\\data_preprocessed_python"
try:
    all_files=glob.glob(path+ "/*.dat") # gets all data files 获取数据文件
    #visualise eeg plots 可视化脑电图
    ee.visualise_egg(all_files)
    #data transformation and handling 数据转换和处理
    filename="train.csv"
    flag=True
    ...
    ypred_svm=[]
    ypred_knn=[]
    #performing Ml on each label在每个标签上执行Ml
    for i in range(len(encodedLabels)):
        ...
        #Decision Tree决策树
        time_start=time.perf_counter() # to find computation time 计算时间
    ...
    #grouping evaluation metrics for each label 对每个标签的评估指标进行分组
    ...
    #identifying emotions from predicted values of each model根据每个模型的预测值识别情绪
