...
for curr_fold in range(21,33):
    ...
    cnn_test_x_backup = cnn_datasets[split] 
    cnn_test_x = cnn_test_x_backup[test_index]
    test_y_backup = labels[split]
    test_y = test_y_backup[test_index]
    ...
    with tf.Session(config=config) as session:
        ...
        ins = pd.DataFrame({'accuracy': np.mean(test_accuracy),'keep_prob': 1 - dropout_prob,
        	                "epoch": epoch + 1,"learning_rate": learning_rate,
                            "train_sample": train_sample, "test_sample": test_sample,"batch_size":batch_size}, index=[0])

        writer = pd.ExcelWriter("./"+arousal_or_valence+"/"+input_file+"_"+str(curr_fold)+".xlsx")
        ...
        sio.savemat("./" + arousal_or_valence + "/"+"models/"+input_file+"_"+str(curr_fold)+".mat",model_dict)
